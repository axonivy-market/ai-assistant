...
const LOCALHOST_URL_PATTERN = /\b(?:https?:\/\/(?:www\.)?|https?:\/\/|www\.)[a-z0-9]+(?:[.\-][a-z0-9]+)*(:[0-9]{1,5})?(?:\/[^()\s]*)?(?:(?:\([^()]*\))*)\b/ig;
...
// Generate a bold word element for asterisks **
const generateBoldWordComponent = word => {
  return `<b>${word.slice(2, -2)}</b>`;
}


// Generate a bold word element for triple hastags ###
const generateHeaderComponent = word => {
  return `<h3>${word.slice(3)}</h3>`;
}
...
const isUrl = word => (word.match(URL_PATTERN) || word.match(LOCALHOST_URL_PATTERN));
...
const parseLine = line => {
  ...
  // Check if line starts with triple hash ###
  const matchedHeaderLines = [...line.matchAll(HASH_PATTERN)].map(match => match[0]);
  if (matchedHeaderLines) {
    matchedHeaderLines.forEach(word => {
      line = line.replace(word, generateHeaderComponent(word));
    })
  };
...
function initializeResultIFrameAfterLoad(frame) {
  setTimeout( function() {
    ...
    // iFrame.get(0).style.height = '60px';
    ...
    const processItemListInProcessWidget = iFrame.get(0).contentWindow.document.getElementsByClassName('process-start-list-item');
    const itemListInTaskOrCaseWidget = iFrame.get(0).contentWindow.document.getElementsByClassName('dashboard-table--row-style');

    const isProcessWidget = frame.src.includes('Processes');

    if (!frameResult) {
      initializeResultIFrameAfterLoad(frame);
      return;
    }
    ...
    // Add margin-top to process items
    if (isProcessWidget && processItemListInProcessWidget.length > 0) {
      $(processItemListInProcessWidget).css('margin-top','12px');
    }
    ...
    if (frameResult.scrollHeight > 400) {
      iFrame.get(0).style.height = '420px';
    } else {
      iFrame.get(0).style.height = frameResult.scrollHeight + 'px';
    }
