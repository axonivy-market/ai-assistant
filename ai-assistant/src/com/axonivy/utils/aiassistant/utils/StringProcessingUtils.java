...
public final class StringProcessingUtils {
  ...
  private static final Pattern[] DELIMITER_PATTERNS = new Pattern[]{
  ...
  private static final Pattern JSON_MD_PATTERN = Pattern
   ...
   * @return A standardized JSON string, or the original text if no extraction occurred.
   */
  ...
  private static String preProcessJson(String json) {
      ...
      if (json.startsWith(DOUBLE_BRACKETS_START_2)) {
          json = json.substring(DOUBLE_BRACKETS_START_2.length() - 1);
      }
      if (json.endsWith(DOUBLE_BRACKETS_END_2)) {
