...
/**
 * Represents an <a href="https://www.elastic.co/">Elasticsearch</a> index as an
 * embedding store. Current implementation assumes the index uses the cosine
 * distance metric. <br>
 * Supports storing {@link Metadata} and filtering by it using {@link Filter}
 * (provided inside {@link EmbeddingSearchRequest}).
 */
public class IvyElasticSearchEmbeddingStore
    implements EmbeddingStore<TextSegment> {

  private static final Logger log = LoggerFactory
      .getLogger(ElasticsearchEmbeddingStore.class);
   ...
   *                  "default". Index will be created automatically if not
   *                  exists.
   * @param dimension Embedding vector dimension (mandatory when index does not
   *                  exist yet).
   */
  public IvyElasticSearchEmbeddingStore(String serverUrl, String apiKey,
      String userName, String password, String indexName, Integer dimension) {
  ...
  public IvyElasticSearchEmbeddingStore(RestClient restClient, String indexName,
      Integer dimension) {
  ...
  public static class Builder {
    ...
    /**
     * @param restClient Elasticsearch RestClient (optional). Effectively
     *                   overrides all other connection parameters like
     *                   serverUrl, etc.
     * @return builder
     */
    ...
    /**
     * @param dimension Embedding vector dimension (mandatory when index does
     *                  not exist yet).
     * @return builder
     */
    ...
    public IvyElasticSearchEmbeddingStore build() {
      if (restClient != null) {
        return new IvyElasticSearchEmbeddingStore(restClient, indexName,
            dimension);
      } else {
        return new IvyElasticSearchEmbeddingStore(serverUrl, apiKey, userName,
            password, indexName, dimension);
      }
    }
  }
  ...
  public List<String> addAll(List<Embedding> embeddings) {
    List<String> ids = embeddings.stream().map(ignored -> randomUUID())
        .collect(toList());
  ...
  public EmbeddingSearchResult<TextSegment> search(
    ...
    try {
      ...
      SearchResponse<EsDocument> response = client
          .search(
              co.elastic.clients.elasticsearch.core.SearchRequest.of(s -> s
                  .index(indexName).query(q -> q.scriptScore(scriptScoreQuery))
  ...
  private ScriptScoreQuery buildScriptScoreQuery(float[] vector, float minScore,
    ...
    return ScriptScoreQuery.of(q -> q.minScore(minScore).query(query)
        .script(s -> s.inline(InlineScript.of(i -> i
            // The script adds 1.0 to the cosine similarity to prevent the score
            // from being negative.
    ...
    try {
      ...
      if (!response.value()) {
        ...
        client.indices().create(
            c -> c.index(indexName).mappings(getDefaultMappings(dimension)));
      }
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  ...
  private TypeMapping getDefaultMappings(int dimension) {
    ...
    properties.put("vector", Property.of(p -> p.denseVector(DenseVectorProperty
        .of(d -> d.dims(dimension).index(true).similarity("cosine")))));
    return TypeMapping.of(c -> c.properties(properties));
  }

  private void bulkIndex(List<String> ids, List<Embedding> embeddings,
    ...
    for (int i = 0; i < size; i++) {
      ...
      document.setVector(embeddings.get(i).vector());
      document.setText(embedded == null ? null : embedded.get(i).text());
      document.setMetadata(
          embedded == null ? null : embedded.get(i).metadata().toMap());
      bulkBuilder.operations(op -> op.index(
          idx -> idx.index(indexName).id(ids.get(finalI)).document(document)));
    }
  ...
  private void throwIfError(ErrorCause errorCause) {
    if (errorCause != null) {
      throw new ElasticsearchRequestFailedException(
          "error:\n" + BusinessEntityConverter.entityToJsonValue(errorCause));
    }
  }
  ...
  private void removeByQuery(Query query) {
    try {
      DeleteByQueryResponse response = client
          .deleteByQuery(delete -> delete.index(indexName).query(query));
  ...
  private void bulkRemove(Collection<String> ids) throws IOException {
    ...
    for (String id : ids) {
      bulkBuilder
          .operations(op -> op.delete(dlt -> dlt.index(indexName).id(id)));
    }
    BulkResponse response = client.bulk(bulkBuilder.build());
    handleBulkResponseErrors(response);
  }

  private List<EmbeddingMatch<TextSegment>> toMatches(
      SearchResponse<EsDocument> response) {
  ...
  public EmbeddingSearchResult<TextSegment> searchExactKnn(
    ...
    try {
      ...
      // Combined Script Score query and Knn query to produce an Exact Knn
      // search query
      // see
      // https://www.elastic.co/guide/en/elasticsearch/reference/current/knn-search.html#exact-knn
      SearchResponse<EsDocument> response = client.search(
  ...
  private KnnQuery buildKnnQuery(float[] vectors, int numberOfCandidates,
      Filter filter) {
    Query query;
    if (filter == null) {
      query = Query.of(q -> q.matchAll(m -> m));
    ...
    return KnnQuery.of(s -> s.field("vector").numCandidates(numberOfCandidates)
        .k(numberOfCandidates).queryVector(vectorList).filter(query));
  }
