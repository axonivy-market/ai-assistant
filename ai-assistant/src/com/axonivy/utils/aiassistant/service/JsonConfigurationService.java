...
public abstract class JsonConfigurationService<T extends AbstractConfiguration> {
  ...
  public T findById(String id) {
    T entity = getPrivateConfig().stream().filter(e -> e.getId().equals(id))
        .findFirst().orElse(null);
    if (entity != null) {
      return entity;
    } else {
      return getPublicConfig().stream().filter(e -> e.getId().equals(id))
          .findFirst().orElse(null);
    }
  }
  ...
  public List<T> getPrivateConfig() {
    ...
    return Optional.ofNullable(convertToLatestVersion(jsonValue)).orElse(
        BusinessEntityConverter.jsonValueToEntities(jsonValue, getType()));
  }
